...
namespace FRIGGA_NAMESPACE
{
    namespace
    {
        ...
        ImU32 ColorFromLight(const glm::vec3 &color)
        {
        ...
        void DrawCircle(ImDrawList *drawList, const glm::mat4 &viewProj, const ImVec2 &imageMin,
                        ...
                        const glm::vec3 &axisY, float radius, ImU32 color, int segments = 24)
        {
            for(int i = 0; i < segments; ++i)
            {
                ...
                DrawSegment(drawList, viewProj, imageMin, imageSize, p0, p1, color);
            }
        }
        ...
        void DrawCone(ImDrawList *drawList, const glm::mat4 &viewProj, const ImVec2 &imageMin,
                      ...
                      float length, float angleRad, ImU32 color)
        {
            ...
            for(int i = 0; i < spokes; ++i)
            {
                ...
                DrawSegment(drawList, viewProj, imageMin, imageSize, tip, edge, color);
            }
                      ...
                      const glm::vec3 &tangent, float halfWidth, float halfHeight, ImU32 color)
        {
            ...
            DrawSegment(drawList, viewProj, imageMin, imageSize, c0, c1, color, 2.0f);
            DrawSegment(drawList, viewProj, imageMin, imageSize, c1, c2, color, 2.0f);
            DrawSegment(drawList, viewProj, imageMin, imageSize, c2, c3, color, 2.0f);
            DrawSegment(drawList, viewProj, imageMin, imageSize, c3, c0, color, 2.0f);
                              ...
                              const ImVec2 &imageMin, const ImVec2 &imageSize)
    {
        ...
        registry->CreateMutation()->Each<TransformComponent, LightComponent>(
            [&](auto, TransformComponent &transform, LightComponent &light) {
                const ImU32 color = ColorFromLight(light.color);
                ...
                DrawCross(drawList, viewProj, imageMin, imageSize, pos, 0.2f, color);
                ...
                case fra::LightType::Point:
                {
                    ...
                    DrawCircle(drawList, viewProj, imageMin, imageSize, pos, {1, 0, 0}, {0, 1, 0},
                               ring, color);
                    DrawCircle(drawList, viewProj, imageMin, imageSize, pos, {1, 0, 0}, {0, 0, 1},
                               ring, color);
                    DrawCircle(drawList, viewProj, imageMin, imageSize, pos, {0, 1, 0}, {0, 0, 1},
                               ring, color);
                    break;
                }
                ...
                case fra::LightType::Spot:
                {
                    ...
                    DrawCone(drawList, viewProj, imageMin, imageSize, pos, safeDir, length,
                             glm::radians(std::max(light.outerAngleDegrees, 1.0f)), color);
                    break;
                }
                case fra::LightType::Area:
                    DrawRect(drawList, viewProj, imageMin, imageSize, pos, safeDir, tangent,
                             std::max(light.halfWidth, 0.01f), std::max(light.halfHeight, 0.01f),
                             color);
                    break;
                }
            });
