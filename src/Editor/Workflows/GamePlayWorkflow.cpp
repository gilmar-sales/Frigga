...
void GamePlayWorkflow::buildDefaultDockLayout(ImGuiID dockspaceId)
{
    ...
    const auto modules    = EditorDock::WindowId("Modules");
    const auto logs       = EditorDock::WindowId("Logs");
    ...
    ImGui::DockBuilderDockWindow(modules.c_str(), bottomId);
    ImGui::DockBuilderDockWindow(logs.c_str(), bottomId);
